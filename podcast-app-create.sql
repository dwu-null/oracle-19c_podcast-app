...
CREATE SEQUENCE idpcharge_seq
 START WITH     100
 INCREMENT BY   1
 NOCACHE
 NOCYCLE;

CREATE SEQUENCE idpepisode_pcharge_seq
 START WITH     100
 INCREMENT BY   1
 NOCACHE
 NOCYCLE;
...
CREATE TABLE PCHARGES(
	premium_charge_id NUMBER DEFAULT idpcharge_seq.NEXTVAL,
	premium_charge decimal(5, 2) NOT NULL,
	PRIMARY KEY (premium_charge_id)
);

CREATE TABLE PEPISODES_PCHARGES(
	pepisode_pcharge_id NUMBER DEFAULT idpepisode_pcharge_seq.NEXTVAL,
	paid_episode_id NUMBER NOT NULL,
	premium_charge_id NUMBER NOT NULL,
	start_time TIMESTAMP NOT NULL,
	end_time TIMESTAMP,
	note VARCHAR2(50) DEFAULT NULL,
	PRIMARY KEY (pepisode_pcharge_id),
	CONSTRAINT fk_p_e_p_c_pe1 FOREIGN KEY (paid_episode_id) REFERENCES PEPISODES (paid_episode_id),
	CONSTRAINT fk_p_e_p_c_pc1 FOREIGN KEY (premium_charge_id) REFERENCES PCHARGES (premium_charge_id)
);
...
--
ALTER TABLE SHOWS
ADD isNameDeleted VARCHAR2(5) DEFAULT 'FALSE';

ALTER TABLE EPISODES
ADD isParticipantDeleted VARCHAR2(5) DEFAULT 'FALSE';

ALTER TABLE PEPISODES
ADD isChargeRemoved VARCHAR2(5) DEFAULT 'FALSE';
...
SELECT PEPISODES.paid_episode_id, PCHARGES.premium_charge, PEPISODES.episode_id
FROM PEPISODES
     LEFT JOIN PEPISODES_PCHARGES
	ON PEPISODES.paid_episode_id = PEPISODES_PCHARGES.paid_episode_id
     LEFT JOIN PCHARGES
	ON PEPISODES_PCHARGES.premium_charge_id = PCHARGES.premium_charge_id
	 RIGHT JOIN EPISODES
	ON PEPISODES.episode_id = EPISODES.episode_id
WHERE PEPISODES_PCHARGES.end_time IS NULL;
...
CREATE OR REPLACE PROCEDURE sp_change_eparticipant (episode_id IN NUMBER,
...
BEGIN
	UPDATE EPISODES_PARTICIPANTS -- set the end time for current participant
		SET end_time = SYSDATE
		WHERE EPISODES_PARTICIPANTS.episode_id = episode_id
		AND end_time IS NULL;
	
	IF pname IS NOT NULL AND role IS NOT NULL THEN -- insert / update the participant
		...
		INSERT INTO EPISODES_PARTICIPANTS VALUES -- set the start time for new participant
			...
			NULL,
			NULL);
	ELSE -- delete the participant
		UPDATE EPISODES
		SET isParticipantDeleted = 'TRUE'
		WHERE EPISODES.episode_id = episode_id;
	END IF;
END;
/
...
BEGIN
	...
	ELSIF DELETING THEN -- no participant exist after deleting
...
CREATE OR REPLACE PROCEDURE sp_change_pcharge (pepisode_id IN NUMBER,
											 pcharge IN VARCHAR2 DEFAULT NULL) AS
	var_pch_seq NUMBER; -- premium_charge_id (PK) used in TABLE PCHARGES
	var_pp_seq NUMBER; -- pepisode_pcharge_id (PK) used in TABLE PEPISODES_PCHARGES
BEGIN
	UPDATE PEPISODES_PCHARGES -- set the end time for current premium charge
		SET end_time = SYSDATE
		WHERE PEPISODES_PCHARGES.paid_episode_id = pepisode_id
		AND end_time IS NULL;
	
	IF pcharge IS NOT NULL THEN -- insert / update the premium charge
		var_pch_seq := idpcharge_seq.NEXTVAL;
		var_pp_seq := idpepisode_pcharge_seq.NEXTVAL;
		
		INSERT INTO PCHARGES VALUES -- add new premium charge to TABLE PCHARGES
			(var_pch_seq, pcharge);
		INSERT INTO PEPISODES_PCHARGES VALUES -- set the start time for new premium charge
			(var_pp_seq,
			pepisode_id,
			var_pch_seq,
			SYSDATE, 
			NULL,
			NULL);
	ELSE -- delete the premium charge
		UPDATE PEPISODES
		SET isChargeRemoved = 'TRUE'
		WHERE PEPISODES.paid_episode_id = pepisode_id;
	END IF;
END;
/
...
DECLARE
	...
	var_pcharge VARCHAR2(50); -- premium_charge used in TABLE PCHARGES
...
BEGIN
	IF INSERTING THEN
		...
		INSERT INTO PEPISODES (paid_episode_id, episode_id)
				VALUES (var_idpepisode, var_idepisode); -- create a new episode
	ELSIF DELETING THEN -- no premium charge exist after deleting
	...
	sp_change_pcharge (var_idpepisode, var_pcharge);
END;
/
